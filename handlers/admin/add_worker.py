...
async def add_worker(callback: CallbackQuery, state: FSMContext):
    ...
    kb.add(InlineKeyboardButton(text='Отмена ⬅️', callback_data='cancel'))

    await callback.message.edit_text("☑️ Введите Telegram ID воркера:", reply_markup=kb.as_markup())
...
async def process_telegram_id(message: Message, state: FSMContext):
    ...
    await message.answer("☑️ Теперь введите имя воркера:")
...
async def process_name(message: Message, state: FSMContext):
    ...
    kb = InlineKeyboardBuilder()
    kb.add(InlineKeyboardButton(text="↩️ К Админ Панели", callback_data='admin_panel'))
